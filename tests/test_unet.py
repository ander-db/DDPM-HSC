...
def test_unet_with_time_embeddings(unet_model_with_time):
    ...
    assert len(unet_model_with_time.time_embeddings) == len(
        unet_model_with_time.encoder_channel_layers
    ) + len(unet_model_with_time.decoder_channel_layers) + 1 # +1 for the mid layer
...
def test_unet_encoder_decoder_structure(unet_model_with_time):
    assert (
        ...
        == len(unet_model_with_time.encoder_channel_layers) - 1
    )
    assert len(unet_model_with_time.decoder) == len(
        unet_model_with_time.decoder_channel_layers
    )
